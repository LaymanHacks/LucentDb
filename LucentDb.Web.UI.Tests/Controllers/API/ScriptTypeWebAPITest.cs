...
{
    [TestClass()]
    public class ScriptTypeApiControllerTests
    {
        
        private Mock<IScriptTypeRepository> _repository;

        private List<ScriptType> _repositoryList = new List<ScriptType>
        {
        //TODO Initialize test data
            new ScriptType()
        };

        private ScriptTypeApiController _target;
        
        [TestInitialize]
        public void Init()
        {
            _repository = new Mock<IScriptTypeRepository>();
            _target = new ScriptTypeApiController(_repository.Object)
            {
                Request = new HttpRequestMessage { RequestUri = new Uri("http://localhost/api/ScriptTypes") }
            };
        ...
        [TestMethod()]
        public void Update_Should_Update_A_ScriptType() 
        {
            _repository
                 .Setup(it => it.Update(It.IsAny<String>(), It.IsAny<Boolean>(), It.IsAny<Int32>()))
                 .Callback<String, Boolean, Int32>((name, isActive, id) => 
            { 
                 var tScriptType = _repositoryList.Find(x => x.Id==id);
                 tScriptType.Name = name; 
                 tScriptType.IsActive = isActive; 
            });
            var tempScriptType = _repositoryList.Find(x => x.Id==id);
            var testScriptType = new ScriptType {
                 Id = tempScriptType.Id, 
                 Name = tempScriptType.Name, 
                 IsActive = tempScriptType.IsActive};
            
            //TODO change something on testScriptType
            //testScriptType.oldValue = newValue; 
            _target.Update(testScriptType);
        ...
        [TestMethod()]
        public void Delete_Should_Delete_A_ScriptType() 
        {
            _repository
        ...
        [TestMethod()]
        public void Insert_Should_Insert_A_ScriptType() 
        {
            _repository
                 .Setup(it => it.Insert(It.IsAny<String>(), It.IsAny<Boolean>()))
                 .Returns<String, Boolean>((name, isActive) => 
            { 
                 _repositoryList.Add(new  ScriptType (name, isActive));
            });
            
            //TODO insert values 
            _target.Insert(new ScriptType (name, isActive));
        ...
        {
            PagedResult<ScriptType> expectedResult;
            ...
            _repository
                 ...
                 { 
                      ...
                      {
                          case  "Id":
                              query = new List<ScriptType>(query.OrderBy(q => q.Id));
                              break;
                          case  "Name":
                              query = new List<ScriptType>(query.OrderBy(q => q.Name));
                              break;
                          case  "IsActive":
                              query = new List<ScriptType>(query.OrderBy(q => q.IsActive));
                              break;                      }
